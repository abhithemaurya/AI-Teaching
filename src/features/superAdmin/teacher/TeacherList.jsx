    ...
    <div className="border border-gray-100 rounded-2xl p-4 bg-white shadow-sm">
      ...
      <div className="flex items-center gap-2 mb-3">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-medium transition active:scale-95 ${isApproved
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-medium transition active:scale-95 ${isRejected
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        ...
        >
          <span
            className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${t.isActive ? "translate-x-5" : "translate-x-0"}`}
          />
        </button>
      </div>
    </div>
...
const TeacherList = () => {
  ...
  const hasActiveFilters =
    search || filterStatus || filterApproval || filterActive;
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      await teacherApproval(id, action);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-80 rounded-2xl shadow-xl p-6 border border-gray-100">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setDeleteId(null)}
                className="flex-1 py-2 text-sm rounded-xl border border-gray-200 hover:bg-gray-50"
              >
                Cancel
              </button>
              <button
      ...
      <div className="px-4 py-4 border-b border-gray-100 space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Manage teachers, approvals and account access
            </p>
          </div>
          <button
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="text-xs border border-gray-200 rounded-lg px-2.5 py-1.5 bg-gray-50 text-gray-700 outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer"
          >
          ...
          <span className="text-xs text-gray-400 ml-auto">
            {filteredTeachers.length} of {teacher.length}
          </span>
        </div>
      </div>
      ...
      <div className="md:hidden">
        {loading ? (
          ...
          <p className="py-12 text-center text-xs text-gray-300">
            No teachers found.
          </p>
        ) : (
      ...
      <div className="hidden md:block overflow-x-auto">
        ...
        >
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={10} className="py-16 text-center text-gray-300">
                  Loading…
                </td>
              </tr>
            ) : filteredTeachers.length === 0 ? (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded bg-blue-50 text-blue-500 text-[10px] font-semibold uppercase tracking-wide">
                        {t.role}
                      </span>
                    </td>
                        ...
                        {status !== "APPROVE" && status !== "REJECT" && (
                          <>
                            <button
                              ...
                              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-medium bg-white border border-emerald-300 text-emerald-600 hover:bg-emerald-50 transition active:scale-95"
                            >
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-medium bg-white border border-red-200 text-red-500 hover:bg-red-50 transition active:scale-95"
                            >
                              <X size={10} /> Reject
                            </button>
                          </>
      ...
      {openCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md">
            <button
              onClick={() => setOpenCreate(false)}
              className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center text-gray-500 hover:text-black"
            >
              ✕
            </button>
            <CreateTeacherForm />
